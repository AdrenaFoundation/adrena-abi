...
pub struct AutonomMarketOpeningData {
    ...
    // Per-feed windows, parallel to `feeds`. `feed_market_*[i]` is the
    // open/close window for `feeds[i]`. Lets a single Autonom pool gate
...
impl AutonomMarketOpeningData {
    ...
    ///
    /// `feeds` and `market_close_affected_feeds` are concatenated without a
    /// length prefix; encoding is trusted-signer safe and documented in the
    /// on-chain program. Layout:
    ///   - market_open_timestamp (i64 LE)
    ///   - market_close_timestamp (i64 LE)
    ///   - feeds (u8 sequence)
    ///   - market_close_affected_feeds (u8 sequence)
    ///   - feed_market_open_timestamps (i64 LE sequence)
    ///   - feed_market_close_timestamps (i64 LE sequence)
    ...
    /// bytes (backward-compatible with the legacy per-pool layout).
